...
# ---------------------------------------------------------------------
# הבלוק צומצם ב-16.9.2026 (סבב שני) לסרגל, אחוז, ודקות — שורת הרמה
# במילים ושורת "הגבוה ביותר" הוסרו בבקשת המשתמש. מה שנשאר עדיין נושא
# את המידע בלי צבע: האחוז עצמו ומיקום המילוי בסרגל.
...
check("הבלוק הוא שתי שורות בלבד", len(text.split("\n")) == 2, f"-> {text!r}")
check("ובלי שורת רמה במילים", "בטווח" not in text)
...
check("ומספר ה-sessions *לא* מופיע", "sessions" not in text and "session ·" not in text,
      f"-> {text!r}")
check("ושורת ה-session הגבוה הוסרה", "הגבוה ביותר" not in text)
...
check("מעל 100% -> הסרגל אדום והאחוז האמיתי מוצג", "🟥" * 10 in over and "145%" in over,
      f"-> {over!r}")
check("ובלי טקסט פרשני", "חשיפה מלאה" not in over and "בטווח" not in over)
...
check("ולא נוספה שורת פרשנות", "בטווח" not in text and "חשיפה מלאה" not in text)
check("עדיין מופיעה התוצאה של ה-session עצמו", "דקות בחיפה" in text)
check("ושורת התקציב הוסרה", "מדד חשיפה:" not in text, f"-> {text!r}")
...
check("וההודעה כולה קצרה — חמש שורות", len(text.split("\n")) == 5, f"-> {text!r}")
