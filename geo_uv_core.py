...
# ---------------------------------------------------------------------
# איזה SPF צריך — היפוך הנוסחה
# ---------------------------------------------------------------------
# נוסף 16.9.2026. safe_exposure_minutes עונה "כמה זמן מותר לי עם
# ה-SPF הזה"; כאן הכיוון ההפוך, שהוא מה שהמשתמש באמת שואל לפני
# שהוא יוצא: "איזה קרם למרוח".
#
#   safe = (200/UV) × skin_factor × effective_spf
#   effective_spf = 1 + (SPF − 1) × 0.4
#
# ולכן, בהינתן משך שרוצים לכסות:
#   effective_needed = משך × UV / (200 × skin_factor)
#   SPF = 1 + (effective_needed − 1) / 0.4
#
# **רצפה של SPF 30, ולא מה שהמתמטיקה נותנת.** הנוסחה אומרת שלרוב
# המקרים SPF 15 מספיק לשעתיים, וזה נכון לפיה — אבל ההמלצה המקובלת
# של גופי בריאות היא 30 כמינימום, ואפליקציה שאומרת 15 כשהקונצנזוס
# אומר 30 ממעיטה בהמלצה. המתמטיקה יכולה רק להעלות מ-30, לא להוריד.
#
# זה גם סוגר פריט פתוח מסקירת 20.8.2026: "לצוות את מדד החשיפה
# בהמלצה ממשית ולא רק במספר".

SUNSCREEN_HORIZON_MINUTES = 120

# מה שנמכר בפועל. 30 הוא הרצפה המומלצת, ולכן גם הערך הראשון.
SUNSCREEN_SHELF_SPF = (30, 50)


def spf_needed_for(uv_index: float, skin_type: int, minutes: float) -> float | None:
    """
    ה-SPF המתויג שנדרש כדי לכסות `minutes` בשמש. 0.0 = לא נדרש קרם
    כלל. None = אין קרינה (UV אפס), ואז אין מה להמליץ.
    """
    if uv_index <= 0:
        return None
    # ברירת מחדל לסוג העור הבהיר ביותר, כמו בכל שאר מקומות ה"לא
    # יודעים" (16.9.2026) — טעות לכיוון "תמרח" ולא לכיוון "אתה בסדר".
    factor = SKIN_TYPE_FACTOR.get(skin_type, SKIN_TYPE_FACTOR[1])
    needed_effective = minutes * uv_index / (200 * factor)
    if needed_effective <= 1:
        return 0.0
    return 1 + (needed_effective - 1) / 0.4


def spf_on_shelf(spf_needed: float | None) -> int | None:
    """
    ה-SPF הנמכר הקרוב שמכסה את הנדרש, עם רצפה של 30.

    0 -> לא נדרש קרם. None -> אין SPF נמכר שמכסה (או שאין קרינה).
    שימו לב: בטווח של שעתיים הענף הזה לא נדרך *אף פעם* — גם UV 12
    עם סוג עור 1 דורש SPF 50 בלבד. הוא קיים כשומר, ויתעורר אם נוסיף
    בחירת משך ארוך יותר (חמש שעות ב-UV 11 דורשות SPF 81).
    """
    if spf_needed is None:
        return None
    if spf_needed == 0.0:
        return 0
    for shelf in SUNSCREEN_SHELF_SPF:
        if spf_needed <= shelf:
            return shelf
    return None
...
def daily_summary_he(
    ...
    """
    בלוק הסיכום היומי: סרגל, אחוז, ודקות בשמש. פונקציה טהורה —
    bot_commands.py שולף את הנתונים, וה-Worker של השקיעה מייצר את
    אותו טקסט בדיוק מ-logic.ts (נבדק מול fixture שנוצר מכאן).

    16.9.2026, סבב שני: הבלוק כלל גם שורת רמה במילים ("בטווח הגבוה.
    עוד 29% עד חשיפה מלאה.") וגם "הגבוה ביותר: חיפה, 41%". שתיהן
    הוסרו בבקשת המשתמש — ההודעה כולה הייתה תשע שורות על session של
    אפס דקות. מה שנשאר נושא את המידע בלי צבע בכל מקרה: האחוז עצמו
    ומיקום המילוי בסרגל. מה שאבד הוא הפרשנות (ש-70 הוא "גבוה"), והיא
    קיימת בדשבורד.
